...
**macOS 优先、本地优先的 AI 截图工具。截图、取字、翻译、长截图、钉图与标注，一步完成。**
...
「拓」取自中国传统的拓印：把碑石、器物上的信息，准确地转移到纸上。Ta 想做同一件事——把屏幕上的文字、图像和上下文快速“拓”下来，并立即变成可以复制、翻译、标注、保存和继续使用的内容。

中文名为「拓」，官方读音为 `tà`；英文名为 **Ta**。

## 为什么做这件事

截图工具已经很多，但真实工作流仍然被切得很碎：截图后要打开 OCR 工具取字，翻译时再切到另一个应用，长聊天记录需要反复拼接，做公众号配图还要进入图片编辑器标注和美化。

Ta 的目标不是简单加一个 AI 按钮，而是让截图成为信息处理的入口：

- 截完即可取字或复制图片，不必先保存文件；
- OCR 与多模态模型是两条独立路径，可按任务和隐私需求选择；
- 本地识别优先，低置信度结果只有在明确确认后才上传；
- 截图后可以继续翻译、钉图、标注、长图拼接或保存；
- 常用动作都有全局快捷键，并且可以自定义。
...
欢迎提交 Issue、功能建议和 Pull Request。开始修改前请阅读 [CONTRIBUTING.md](./CONTRIBUTING.md)，并尽量为行为变化补充测试或验收步骤。
